    use super::{
        ...
        ModalProfile, ModalProfileId, ModalResonator, RealtimeModeCountEstimate, RustAmount,
    ...
    fn assert_size_scaling_direction(profile_id: ModalProfileId) {
        ...
        let (smaller_output, smaller_summary) = renderer.render(
            ModalResonator::with_profile_and_size(profile_id, smaller_scale),
        );
        let (larger_output, larger_summary) = renderer.render(
            ModalResonator::with_profile_and_size(profile_id, larger_scale),
        );
