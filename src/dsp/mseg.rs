//! Multistage envelope generator used by strike, scrape, and sustain voices.
//!
...
impl MSEG {
    ...
    /// Configure for continuous friction or scrape behavior.
...
mod tests {
    ...
    fn mseg_advances_through_stages() {
        ...
        assert!(stages_seen.contains(&Stage::Onset), "Should see Onset stage");
        assert!(stages_seen.contains(&Stage::Attack), "Should see Attack stage");
    ...
    fn mseg_reaches_peak() {
        ...
        assert!(max_level > 0.5, "MSEG should reach significant level, got {}", max_level);
    }
    ...
    fn curve_application() {
        ...
        assert!(convex < 0.5, "Convex curve should be below linear at midpoint");

        let concave = MSEG::apply_curve(0.5, 0.5);
        assert!(concave > 0.5, "Concave curve should be above linear at midpoint");
    }
    ...
    fn velocity_affects_level() {
        ...
        let max_low: f32 = (0..100).map(|_| mseg_low.process_sample()).fold(0.0, f32::max);
        let max_high: f32 = (0..100).map(|_| mseg_high.process_sample()).fold(0.0, f32::max);

        assert!(max_high > max_low, "High velocity should give higher peak: high={}, low={}", max_high, max_low);
    }
}
