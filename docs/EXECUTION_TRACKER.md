# Corrosion Execution Tracker

## How to Use This Tracker
- Each top-level gate is a hard milestone gate.
- Do not mark a gate complete until its implementation tasks, evidence tasks, and gate review tasks are all complete.
...
## Gate 0 — Research Prototype
- [ ] Gate 0 complete
  - [ ] Build an offline DSP renderer in Rust or Python
  - [ ] Implement the second-order modal resonator prototype
  - [ ] Create deterministic excitation input for repeatable testing
  - [ ] Create initial pipe modal profile
  - [ ] Create initial plate modal profile
  - [ ] Create initial tank modal profile
  - [ ] Tune object families so they are clearly distinct by ear
  - [ ] Test size scaling against pitch and resonance behavior
  - [ ] Prototype rust-based decay and brightness loss
  - [ ] Prototype damage-driven detuning and roughness
  - [ ] Estimate safe mode counts for real-time use
  - [ ] Identify likely CPU hot spots
  - [ ] Render comparison WAVs for pipe, plate, and tank
  - [ ] Render low/high rust examples
  - [ ] Render low/high damage examples
